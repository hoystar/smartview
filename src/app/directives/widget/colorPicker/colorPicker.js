...
var moduleName = "app.colorPicker";
  ...
  function($compile){
    return {
      ...
      link: function(scope,element,attrs){
            scope.selectedFontColor = attrs.selectedFontcolor;//'#f00';
            scope.$on('colorPicked', function(event, color) {
                scope.selectedFontColor = scope.initcolor = color;       
            });

        let unWatcher = scope.$watch("initcolor", function(newValue, oldValue) {
          if (newValue) {
                scope.selectedFontColor = newValue;
 /*           if(scope.data.folderCanvas && scope.data.folderCanvas.data){
              layout.draw(scope.data.folderCanvas.data, isRecommend);   
              unWatcher();    
            }*/
            unWatcher();     
          }
